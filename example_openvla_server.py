"""
Provide a lightweight server implementation for deploying OpenVLA models (through the HF AutoClass API) over a
REST API. This script implements *just* the server.

Usage:
```
python example_openvla_server.py --port 8000
bore local 8000 --to bore.pub  # expose local port 8000 to open web through bore.pub
```

Dependencies:
    => OpenVLA depencencies from https://github.com/openvla/openvla?tab=readme-ov-file#installation
    => `pip install uvicorn fastapi json-numpy prismatic`

Example Client (Standalone) Usage (assuming a server running on 0.0.0.0:8000):
